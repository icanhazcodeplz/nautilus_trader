...
class CustomExecTesterConfig(StrategyConfig, frozen=True):
    """
...
class CustomExecTester(Strategy):
    """
    ...
    def __init__(self, config: CustomExecTesterConfig) -> None:
    ...
    def on_trade_tick(self, trade: TradeTick) -> None:
        ...
        """
        if self.config.log_data:
            msg = f"{pd.Timestamp(trade.ts_init) - pd.Timedelta(hours=4)} | {trade.size} at {trade.price}"
            self.log.info(msg, LogColor.CYAN)
    ...
    def open_position(self, net_qty: Decimal) -> None:
        ...
        orders = [
            self.order_factory.limit(
                ...
                price=self.instrument.make_price(259),
            ...
            self.order_factory.limit(
                ...
                price=self.instrument.make_price(270),
                quantity=self.instrument.make_qty(5),
                time_in_force=self.config.open_position_time_in_force,
            ),
            # self.order_factory.limit(
            #     instrument_id=self.config.instrument_id,
            #     order_side=OrderSide.BUY,
            #     price=self.instrument.make_price(260),
            #     quantity=self.instrument.make_qty(10),
            #     time_in_force=self.config.open_position_time_in_force,
            # ),
        ]
        ...
        for order in orders:
            self.submit_order(order, client_id=self.client_id, params=self.config.order_params)
            print("SLEEPING")
            sleep(3)
