...
from nautilus_trader.adapters.okx.config import AlpacaExecClientConfig
from nautilus_trader.adapters.okx.constants import Alpaca_VENUE
from nautilus_trader.adapters.okx.providers import AlpacaInstrumentProvider
...
from nautilus_trader.core.nautilus_pyo3 import AlpacaInstrumentType
from nautilus_trader.core.nautilus_pyo3 import AlpacaTradeMode
...
class AlpacaExecutionClient(LiveExecutionClient):
    """
    Provides an execution client for the Alpaca centralized crypto exchange.
    ...
    def __init__(
        ...
        if instrument_provider.instrument_types == (AlpacaInstrumentType.SPOT,):
        ...
        # WebSocket API
        self._ws_client = nautilus_pyo3.AlpacaWebSocketClient.with_credentials(
            url=config.base_url_ws or nautilus_pyo3.get_okx_ws_url_private(config.is_demo),
            account_id=self.pyo3_account_id,
        )
        self._ws_client_futures: set[asyncio.Future] = set()

        self._ws_business_client = nautilus_pyo3.AlpacaWebSocketClient.with_credentials(
        ...
        if account_type == AccountType.CASH:
            self._trade_mode = AlpacaTradeMode.CASH
        else:
            # TODO: Initially support isolated margin only
            self._trade_mode = AlpacaTradeMode.ISOLATED

    @property
    def okx_instrument_provider(self) -> AlpacaInstrumentProvider:
    ...
    async def _submit_order_websocket(self, command: SubmitOrder) -> None:
        ...
        if command.params:
            ...
            if td_mode_str:
                try:
                    td_mode = AlpacaTradeMode(td_mode_str)
                except ValueError:
                    self._log.warning(
                        f"Failed to parse AlpacaTradeMode: Valid modes are 'cash', 'isolated', 'cross', 'spot_isolated', "
    ...
    async def _submit_algo_order_http(self, command: SubmitOrder) -> None:
        ...
        try:
            ...
            if response.get("s_code") and response["s_code"] != "0":
                raise ValueError(f"Alpaca API error: {response.get('s_msg', 'Unknown error')}")
    ...
    async def _cancel_all_orders_mass_cancel(self, command: CancelAllOrders) -> None:
        # Use Alpaca's mass-cancel WebSocket endpoint for market makers
        pyo3_instrument_id = nautilus_pyo3.InstrumentId.from_str(command.instrument_id.value)
    ...
    def _handle_msg(self, msg: Any) -> None:
        if isinstance(msg, nautilus_pyo3.AlpacaWebSocketError):
