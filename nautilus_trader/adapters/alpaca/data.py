...
class AlpacaDataClient(LiveMarketDataClient):
    """
    Provides a data client for the Alpaca broker.
    ...
    def __init__(
        ...
        self._ws_client = nautilus_pyo3.AlpacaWebSocketClient(
        ...
        self._ws_business_client = nautilus_pyo3.AlpacaWebSocketClient(
    ...
    async def _subscribe_order_book_deltas(self, command: SubscribeOrderBook) -> None:
        if command.book_type != BookType.L2_MBP:
            self._log.warning(
                f"Book type {book_type_to_str(command.book_type)} not supported by Alpaca, skipping subscription",
            )
            return
    ...
    async def _request_quote_ticks(self, request: RequestQuoteTicks) -> None:
        self._log.error(
            "Cannot request historical quotes: not published by Alpaca. Subscribe to "
