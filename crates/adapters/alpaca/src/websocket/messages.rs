...
use super::enums::{ALPACAWsChannel, ALPACAWsOperation};
use crate::{
    common::{
        enums::{
            ALPACAAlgoOrderType, ALPACABookAction, ALPACACandleConfirm, ALPACAExecType, ALPACAInstrumentType,
            ALPACAOrderStatus, ALPACAOrderType, ALPACAPositionSide, ALPACASide, ALPACATradeMode, ALPACATriggerType,
        },
        parse::{deserialize_empty_string_as_none, deserialize_string_to_u64},
    },
    websocket::enums::ALPACASubscriptionEvent,
};
...
pub struct ALPACAWebSocketError {
    /// Error code from ALPACA (e.g., "50101").
    pub code: String,
...
pub struct ALPACAAuthentication {
    pub op: &'static str,
    pub args: Vec<ALPACAAuthenticationArg>,
}

/// ALPACA WebSocket authentication arguments.
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
...
pub struct ALPACASubscriptionArg {
    pub channel: ALPACAWsChannel,
    pub inst_type: Option<ALPACAInstrumentType>,
...
pub enum ALPACAWebSocketEvent {
    ...
    Subscription {
        event: ALPACASubscriptionEvent,
        arg: ALPACAWebSocketArg,
        #[serde(rename = "connId")]
        conn_id: String,
    },
    ChannelConnCount {
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ALPACAPriceLimitMsg {
    /// Instrument ID.
    pub inst_id: Ustr,
    /// Buy limit price.
    pub buy_lmt: String,
...
pub struct ALPACACandleMsg {
    ...
    pub confirm: ALPACACandleConfirm,
}

/// Open interest data.
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ALPACAOpenInterestMsg {
    /// Instrument ID.
    pub inst_id: Ustr,
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ALPACAOptionSummaryMsg {
    /// Instrument ID.
    pub inst_id: Ustr,
    /// Underlying.
    pub uly: String,
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ALPACAEstimatedPriceMsg {
    /// Instrument ID.
    pub inst_id: Ustr,
...
pub struct ALPACAAlgoOrderMsg {
    ...
    pub trigger_px_type: ALPACATriggerType,
...
mod tests {
    ...
    fn test_deserialize_websocket_arg() {
        ...
        let result: Result<ALPACAWebSocketArg, _> = serde_json::from_str(json_str);
        match result {
            Ok(arg) => {
                assert_eq!(arg.channel, ALPACAWsChannel::Instruments);
                assert_eq!(arg.inst_type, Some(ALPACAInstrumentType::Spot));
    ...
    fn test_deserialize_subscribe_variant_direct() {
        ...
        match result {
            Ok(msg) => {
                ...
                assert_eq!(msg.arg.channel, ALPACAWsChannel::Instruments);
    ...
    fn test_deserialize_subscribe_confirmation() {
        ...
        let result: Result<ALPACAWebSocketEvent, _> = serde_json::from_str(json_str);
        match result {
            Ok(msg) => {
                if let ALPACAWebSocketEvent::Subscription {
                    event,
                    arg,
                    conn_id,
                } = msg
                {
                    assert_eq!(event, ALPACASubscriptionEvent::Subscribe);
                    assert_eq!(arg.channel, ALPACAWsChannel::Instruments);
    ...
    fn test_deserialize_subscribe_with_inst_id() {
        ...
        match result {
            Ok(msg) => {
                ...
                {
                    assert_eq!(event, ALPACASubscriptionEvent::Subscribe);
                    assert_eq!(arg.channel, ALPACAWsChannel::Candle1Minute);
    ...
    fn test_channel_serialization_for_logging() {
        let channel = ALPACAWsChannel::Candle1Minute;
        ...
        let channel = ALPACAWsChannel::BboTbt;
        let serialized = serde_json::to_string(&channel).unwrap();
        let cleaned = serialized.trim_matches('"').to_string();
    ...
    fn test_order_response_with_enum_operation() {
        ...
        let result: Result<ALPACAWebSocketEvent, _> = serde_json::from_str(json_str);
        match result {
            Ok(ALPACAWebSocketEvent::OrderResponse {
                id,
                op,
                code,
                msg,
                ...
                assert_eq!(op, ALPACAWsOperation::Order);
                assert_eq!(code, "0");
                assert_eq!(msg, "");
        ...
        match result {
            Ok(ALPACAWebSocketEvent::OrderResponse {
                ...
                assert_eq!(op, ALPACAWsOperation::CancelOrder);
        ...
        match result {
            Ok(ALPACAWebSocketEvent::OrderResponse {
                ...
                assert_eq!(op, ALPACAWsOperation::AmendOrder);
    ...
    fn test_operation_enum_serialization() {
        let op = ALPACAWsOperation::Order;
        let serialized = serde_json::to_string(&op).unwrap();
        assert_eq!(serialized, "\"order\"");

        let op = ALPACAWsOperation::CancelOrder;
        let serialized = serde_json::to_string(&op).unwrap();
        assert_eq!(serialized, "\"cancel-order\"");

        let op = ALPACAWsOperation::AmendOrder;
        let serialized = serde_json::to_string(&op).unwrap();
        assert_eq!(serialized, "\"amend-order\"");

        let op = ALPACAWsOperation::Subscribe;
        let serialized = serde_json::to_string(&op).unwrap();
    ...
    fn test_order_response_parsing() {
        ...
        let parsed: ALPACAWebSocketEvent = serde_json::from_str(success_response).unwrap();
        ...
        let parsed: ALPACAWebSocketEvent = serde_json::from_str(failure_response).unwrap();
    ...
    fn test_subscription_event_parsing() {
        ...
        let parsed: ALPACAWebSocketEvent = serde_json::from_str(subscription_json).unwrap();
        ...
        match parsed {
            ALPACAWebSocketEvent::Subscription {
                ...
                assert_eq!(
                    event,
                    crate::websocket::enums::ALPACASubscriptionEvent::Subscribe
                );
                assert_eq!(arg.channel, ALPACAWsChannel::Tickers);
    ...
    fn test_login_event_parsing() {
        ...
        let parsed: ALPACAWebSocketEvent = serde_json::from_str(login_success).unwrap();
    ...
    fn test_error_event_parsing() {
        ...
        let parsed: ALPACAWebSocketEvent = serde_json::from_str(error_json).unwrap();
    ...
    fn test_subscription_request_serialization() {
        let subscription = ALPACASubscription {
            ...
            args: vec![ALPACASubscriptionArg {
                channel: ALPACAWsChannel::Tickers,
                inst_type: Some(crate::common::enums::ALPACAInstrumentType::Spot),
                inst_family: None,
    ...
    fn test_error_message_extraction() {
        ...
        for (response_json, expected_msg) in responses {
            let parsed: ALPACAWebSocketEvent = serde_json::from_str(response_json).unwrap();
    ...
    fn test_book_data_parsing() {
        ...
        let parsed: ALPACAWebSocketEvent = serde_json::from_str(book_data_json).unwrap();

        match parsed {
            ALPACAWebSocketEvent::BookData { arg, action, data } => {
                assert_eq!(arg.channel, ALPACAWsChannel::Books);
                assert_eq!(arg.inst_id, Some(Ustr::from("BTC-USDT")));
                assert_eq!(
                    action,
                    super::super::super::common::enums::ALPACABookAction::Snapshot
                );
                assert_eq!(data.len(), 1);
            }
    ...
    fn test_data_event_parsing() {
        ...
        let parsed: ALPACAWebSocketEvent = serde_json::from_str(data_json).unwrap();

        match parsed {
            ALPACAWebSocketEvent::Data { arg, data } => {
                assert_eq!(arg.channel, ALPACAWsChannel::Trades);
                assert_eq!(arg.inst_id, Some(Ustr::from("BTC-USDT")));
    ...
    fn test_order_response_parsing_success() {
        ...
        let parsed: ALPACAWebSocketEvent = serde_json::from_str(order_response_json).unwrap();
