...
//! Request parameter structures for the ALPACA **v5 REST API**.
//!
//! Each struct corresponds 1-to-1 with an ALPACA REST endpoint and is annotated
...
//! the [official ALPACA documentation](https://www.alpaca.com/docs-v5/en/) and, where
//! beneficial, links to the exact endpoint section.  All links point to the
//! English version.
//!
//! Example – building a request for historical trades:
//! ```rust
//! use nautilus_alpaca::http::query::{GetTradesParams, GetTradesParamsBuilder};
//!
...
//! ```
//!
//! Once built these parameter structs are passed to `ALPACAHttpClient::get`/`post`
...
use crate::{
    common::enums::{
        ALPACAInstrumentType, ALPACAOrderStatus, ALPACAOrderType, ALPACAPositionMode, ALPACAPositionSide,
        ALPACATradeMode,
    },
...
pub struct SetPositionModeParams {
    ...
    pub pos_mode: ALPACAPositionMode,
}
...
impl GetCandlesticksParamsBuilder {
    ...
    pub fn build(&mut self) -> Result<GetCandlesticksParams, BuildError> {
        ...
        // ALPACA API doesn't support both 'after' and 'before' parameters together
