...
//! Error structures and enumerations for the ALPACA integration.
//!
//! The JSON error schema is described in the ALPACA documentation under
//! *REST API > Error Codes* – <https://www.alpaca.com/docs-v5/en/#error-codes>.
...
pub struct ALPACAErrorResponse {
    /// The top-level error object included in the ALPACA error response.
    pub error: ALPACAErrorMessage,
}

/// Contains the specific error details provided by the ALPACA API.
#[derive(Clone, Debug, Deserialize)]
pub struct ALPACAErrorMessage {
    /// A human-readable explanation of the error condition.
    pub message: String,
    /// A short identifier or category for the error, as returned by ALPACA.
    pub name: String,
}

/// A typed error enumeration for the ALPACA HTTP client.
#[derive(Debug, Error)]
pub enum ALPACAHttpError {
    ...
    MissingCredentials,
    /// Errors returned directly by ALPACA (non-zero code).
    #[error("ALPACA error {error_code}: {message}")]
    AlpacaError { error_code: String, message: String },
    ...
    /// Any unknown HTTP status or unexpected response from ALPACA.
    #[error("Unexpected HTTP status code {status}: {body}")]
    UnexpectedStatus { status: StatusCode, body: String },
}

impl From<String> for ALPACAHttpError {
    fn from(error: String) -> Self {
        ALPACAHttpError::ValidationError(error)
    }
}
...
impl From<serde_json::Error> for ALPACAHttpError {
    fn from(error: serde_json::Error) -> Self {
        ALPACAHttpError::JsonError(error.to_string())
    }
}
