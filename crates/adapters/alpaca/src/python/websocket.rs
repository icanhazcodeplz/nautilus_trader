...
use crate::{
    common::enums::{ALPACAInstrumentType, ALPACATradeMode},
    websocket::{
        ALPACAWebSocketClient,
        messages::{ExecutionReport, NautilusWsMessage, ALPACAWebSocketError},
    },
};

#[pyo3::pymethods]
impl ALPACAWebSocketError {
    #[getter]
    ...
    fn __repr__(&self) -> String {
        format!(
            "ALPACAWebSocketError(code='{}', message='{}', conn_id={:?}, ts_event={})",
...
impl ALPACAWebSocketClient {
    ...
    fn py_get_subscriptions(&self, instrument_id: InstrumentId) -> Vec<String> {
        ...
        // Convert to ALPACA channel names
        channels
            .iter()
            .map(|c| {
    ...
    fn py_batch_submit_orders<'py>(
        ...
        for obj in orders {
            ...
            let inst_type =
                ALPACAInstrumentType::from_str(&instrument_type).map_err(to_pyvalue_err)?;
            let trade_mode = ALPACATradeMode::from_str(&td_mode).map_err(to_pyvalue_err)?;
