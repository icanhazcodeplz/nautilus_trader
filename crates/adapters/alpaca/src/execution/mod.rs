...
use crate::{
    common::{
        consts::{ALPACA_CONDITIONAL_ORDER_TYPES, ALPACA_VENUE},
        enums::{ALPACAInstrumentType, ALPACATradeMode},
    },
...
impl ALPACAExecutionClient {
    ...
    pub fn new(core: ExecutionClientCore, config: ALPACAExecClientConfig) -> anyhow::Result<Self> {
        let http_client = if config.has_api_credentials() {
            ALPACAHttpClient::with_credentials(
        ...
        .context("failed to construct ALPACA execution websocket client")?;

        let trade_mode = match core.account_type {
            AccountType::Cash => ALPACATradeMode::Cash,
            _ => ALPACATradeMode::Isolated,
        };
    ...
    fn instrument_types(&self) -> Vec<ALPACAInstrumentType> {
        if self.config.instrument_types.is_empty() {
            vec![ALPACAInstrumentType::Spot]
        } else {
    ...
    async fn ensure_instruments_initialized_async(&mut self) -> anyhow::Result<()> {
        ...
        for instrument_type in self.instrument_types() {
            let instruments = self
                ...
                .await
                .with_context(|| {
                    format!("failed to request ALPACA instruments for {instrument_type:?}")
                })?;
    ...
    async fn refresh_account_state(&self) -> anyhow::Result<()> {
        let account_state = self
            ...
            .await
            .context("failed to request ALPACA account state")?;
...
impl ExecutionClient for ALPACAExecutionClient {
    ...
    fn start(&mut self) -> anyhow::Result<()> {
        ...
        self.started = true;
        info!("ALPACA execution client {} started", self.core.client_id);
        Ok(())
    }
    ...
    fn stop(&mut self) -> anyhow::Result<()> {
        ...
        info!("ALPACA execution client {} stopped", self.core.client_id);
        Ok(())
    }
    ...
    fn submit_order_list(
        ...
        warn!(
            "submit_order_list not yet implemented for ALPACA execution client (got {} orders)",
    ...
    fn batch_cancel_orders(
        ...
        for cancel in &cmd.cancels {
            payload.push((
                ALPACAInstrumentType::Spot, // TODO: derive real instrument type
    ...
    fn query_order(
        ...
        debug!(
            "query_order not implemented for ALPACA execution client (client_order_id={})",
            cmd.client_order_id
        );
        Ok(())
    }
...
#[async_trait(?Send)]
impl LiveExecutionClient for ALPACAExecutionClient {
    async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        self.connected = true;
        info!("ALPACA execution client {} connected", self.core.client_id);
        Ok(())
    }
    ...
    async fn disconnect(&mut self) -> anyhow::Result<()> {
        ...
        if let Err(err) = self.ws_client.close().await {
            warn!("Error while closing ALPACA websocket: {err:?}");
        }
        ...
        self.connected = false;
        info!("ALPACA execution client {} disconnected", self.core.client_id);
        Ok(())
    }
...
fn dispatch_ws_message(message: NautilusWsMessage) {
    match message {
        ...
        NautilusWsMessage::Error(err) => {
            warn!(
                "ALPACA websocket error: code={} message={} conn_id={:?}",
                err.code, err.message, err.conn_id
            );
        }
        NautilusWsMessage::Reconnected => {
            info!("ALPACA websocket reconnected");
        }
        ...
        | NautilusWsMessage::Instrument(_) => {
            debug!("Ignoring ALPACA websocket data message");
        }
    }
}
