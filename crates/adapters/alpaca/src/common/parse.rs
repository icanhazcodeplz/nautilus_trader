...
//! Parsing utilities that convert ALPACA payloads into Nautilus domain models.
...
use super::enums::ALPACAContractType;
use crate::{
    common::{
        consts::ALPACA_VENUE,
        enums::{
            ALPACAExecType, ALPACAInstrumentType, ALPACAOrderStatus, ALPACAOrderType, ALPACAPositionSide, ALPACASide,
        },
        models::ALPACAInstrument,
    },
    http::models::{
        ALPACAAccount, ALPACACandlestick, ALPACAIndexTicker, ALPACAMarkPrice, ALPACAOrderHistory, ALPACAPosition,
        ALPACATrade, ALPACATransactionDetail,
    },
    websocket::{enums::ALPACAWsChannel, messages::ALPACAFundingRateMsg},
};

/// Deserializes an empty string into [`None`].
///
/// ALPACA frequently represents *null* string fields as an empty string (`""`).
...
/// Returns the [`ALPACAInstrumentType`] that corresponds to the supplied
/// [`InstrumentAny`].
///
/// # Errors
///
/// Returns an error if the instrument variant is not supported by ALPACA.
pub fn alpaca_instrument_type(instrument: &InstrumentAny) -> anyhow::Result<ALPACAInstrumentType> {
    match instrument {
        InstrumentAny::CurrencyPair(_) => Ok(ALPACAInstrumentType::Spot),
        InstrumentAny::CryptoPerpetual(_) => Ok(ALPACAInstrumentType::Swap),
        InstrumentAny::CryptoFuture(_) => Ok(ALPACAInstrumentType::Futures),
        InstrumentAny::CryptoOption(_) => Ok(ALPACAInstrumentType::Option),
        _ => anyhow::bail!("Invalid instrument type for ALPACA: {instrument:?}"),
    }
}

/// Parses a Nautilus instrument ID from the given ALPACA `symbol` value.
#[must_use]
pub fn parse_instrument_id(symbol: Ustr) -> InstrumentId {
    InstrumentId::new(Symbol::from_ustr_unchecked(symbol), *ALPACA_VENUE)
}

/// Parses a Nautilus client order ID from the given ALPACA `clOrdId` value.
#[must_use]
...
/// Converts a millisecond-based timestamp (as returned by ALPACA) into
/// [`UnixNanos`].
#[must_use]
...
///
/// ALPACA represents *charges* as positive numbers but they reduce the account
...
pub fn parse_fee(value: Option<&str>, currency: Currency) -> anyhow::Result<Money> {
    // ALPACA report positive fees with negative signs (i.e., fee charged)
    let fee_f64 = value.unwrap_or("0").parse::<f64>()?;
    Money::new_checked(-fee_f64, currency)
}

/// Parses ALPACA side to Nautilus aggressor side.
pub fn parse_aggressor_side(side: &Option<ALPACASide>) -> AggressorSide {
    match side {
        Some(ALPACASide::Buy) => AggressorSide::Buyer,
        Some(ALPACASide::Sell) => AggressorSide::Seller,
        None => AggressorSide::NoAggressor,
    }
}

/// Parses ALPACA execution type to Nautilus liquidity side.
pub fn parse_execution_type(liquidity: &Option<ALPACAExecType>) -> LiquiditySide {
    match liquidity {
        Some(ALPACAExecType::Maker) => LiquiditySide::Maker,
        Some(ALPACAExecType::Taker) => LiquiditySide::Taker,
...
/// Parses an ALPACA mark price record into a Nautilus [`MarkPriceUpdate`].
///
/// # Errors
///
...
/// Parses an ALPACA index ticker record into a Nautilus [`IndexPriceUpdate`].
///
/// # Errors
///
...
/// Parses an [`ALPACAFundingRateMsg`] into a [`FundingRateUpdate`].
///
/// # Errors
///
...
/// Parses an ALPACA trade record into a Nautilus [`TradeTick`].
///
/// # Errors
///
...
/// Parses an ALPACA historical candlestick record into a Nautilus [`Bar`].
///
/// # Errors
///
...
/// Parses an ALPACA order history record into a Nautilus [`OrderStatusReport`].
#[allow(clippy::too_many_lines)]
pub fn parse_order_status_report(
    order: &ALPACAOrderHistory,
    ...
    let alpaca_status: ALPACAOrderStatus = order.state;
    let order_status: OrderStatus = alpaca_status.into();
    let alpaca_ord_type: ALPACAOrderType = order.ord_type;
    let order_type: OrderType = alpaca_ord_type.into();
    // Note: ALPACA uses ordType for type and liquidity instructions; time-in-force not explicitly represented here
    ...
    if order.ord_type == ALPACAOrderType::PostOnly {
...
/// Parses an ALPACA position into a Nautilus [`PositionStatusReport`].
///
/// # Panics
///
...
/// Parses an ALPACA transaction detail into a Nautilus `FillReport`.
///
/// # Errors
///
/// This function will return an error if the ALPACA transaction detail cannot be parsed.
pub fn parse_fill_report(
    detail: ALPACATransactionDetail,
    account_id: AccountId,
    instrument_id: InstrumentId,
    price_precision: u8,
    size_precision: u8,
    ...
    Ok(FillReport::new(
        ...
        liquidity_side,
        client_order_id,
        None, // venue_position_id not provided by ALPACA fills
        ts_event,
        ts_init,
...
/// Parses vector messages from ALPACA WebSocket data.
///
...
/// Converts a Nautilus bar specification into the matching ALPACA candle channel.
pub fn bar_spec_as_alpaca_channel(bar_spec: BarSpecification) -> anyhow::Result<ALPACAWsChannel> {
    let channel = match bar_spec {
        BAR_SPEC_1_SECOND_LAST => ALPACAWsChannel::Candle1Second,
        BAR_SPEC_1_MINUTE_LAST => ALPACAWsChannel::Candle1Minute,
        BAR_SPEC_3_MINUTE_LAST => ALPACAWsChannel::Candle3Minute,
        BAR_SPEC_5_MINUTE_LAST => ALPACAWsChannel::Candle5Minute,
        BAR_SPEC_15_MINUTE_LAST => ALPACAWsChannel::Candle15Minute,
        BAR_SPEC_30_MINUTE_LAST => ALPACAWsChannel::Candle30Minute,
        BAR_SPEC_1_HOUR_LAST => ALPACAWsChannel::Candle1Hour,
        BAR_SPEC_2_HOUR_LAST => ALPACAWsChannel::Candle2Hour,
        BAR_SPEC_4_HOUR_LAST => ALPACAWsChannel::Candle4Hour,
        BAR_SPEC_6_HOUR_LAST => ALPACAWsChannel::Candle6Hour,
        BAR_SPEC_12_HOUR_LAST => ALPACAWsChannel::Candle12Hour,
        BAR_SPEC_1_DAY_LAST => ALPACAWsChannel::Candle1Day,
        BAR_SPEC_2_DAY_LAST => ALPACAWsChannel::Candle2Day,
        BAR_SPEC_3_DAY_LAST => ALPACAWsChannel::Candle3Day,
        BAR_SPEC_5_DAY_LAST => ALPACAWsChannel::Candle5Day,
        BAR_SPEC_1_WEEK_LAST => ALPACAWsChannel::Candle1Week,
        BAR_SPEC_1_MONTH_LAST => ALPACAWsChannel::Candle1Month,
        BAR_SPEC_3_MONTH_LAST => ALPACAWsChannel::Candle3Month,
        BAR_SPEC_6_MONTH_LAST => ALPACAWsChannel::Candle6Month,
        BAR_SPEC_12_MONTH_LAST => ALPACAWsChannel::Candle1Year,
        _ => anyhow::bail!("Invalid `BarSpecification` for channel, was {bar_spec}"),
    };
    Ok(channel)
}

/// Converts Nautilus bar specification to ALPACA mark price channel.
pub fn bar_spec_as_alpaca_mark_price_channel(
    bar_spec: BarSpecification,
) -> anyhow::Result<ALPACAWsChannel> {
    let channel = match bar_spec {
        BAR_SPEC_1_SECOND_LAST => ALPACAWsChannel::MarkPriceCandle1Second,
        BAR_SPEC_1_MINUTE_LAST => ALPACAWsChannel::MarkPriceCandle1Minute,
        BAR_SPEC_3_MINUTE_LAST => ALPACAWsChannel::MarkPriceCandle3Minute,
        BAR_SPEC_5_MINUTE_LAST => ALPACAWsChannel::MarkPriceCandle5Minute,
        BAR_SPEC_15_MINUTE_LAST => ALPACAWsChannel::MarkPriceCandle15Minute,
        BAR_SPEC_30_MINUTE_LAST => ALPACAWsChannel::MarkPriceCandle30Minute,
        BAR_SPEC_1_HOUR_LAST => ALPACAWsChannel::MarkPriceCandle1Hour,
        BAR_SPEC_2_HOUR_LAST => ALPACAWsChannel::MarkPriceCandle2Hour,
        BAR_SPEC_4_HOUR_LAST => ALPACAWsChannel::MarkPriceCandle4Hour,
        BAR_SPEC_6_HOUR_LAST => ALPACAWsChannel::MarkPriceCandle6Hour,
        BAR_SPEC_12_HOUR_LAST => ALPACAWsChannel::MarkPriceCandle12Hour,
        BAR_SPEC_1_DAY_LAST => ALPACAWsChannel::MarkPriceCandle1Day,
        BAR_SPEC_2_DAY_LAST => ALPACAWsChannel::MarkPriceCandle2Day,
        BAR_SPEC_3_DAY_LAST => ALPACAWsChannel::MarkPriceCandle3Day,
        BAR_SPEC_5_DAY_LAST => ALPACAWsChannel::MarkPriceCandle5Day,
        BAR_SPEC_1_WEEK_LAST => ALPACAWsChannel::MarkPriceCandle1Week,
        BAR_SPEC_1_MONTH_LAST => ALPACAWsChannel::MarkPriceCandle1Month,
        BAR_SPEC_3_MONTH_LAST => ALPACAWsChannel::MarkPriceCandle3Month,
        _ => anyhow::bail!("Invalid `BarSpecification` for mark price channel, was {bar_spec}"),
    };
    Ok(channel)
}

/// Converts Nautilus bar specification to ALPACA timeframe string.
pub fn bar_spec_as_alpaca_timeframe(bar_spec: BarSpecification) -> anyhow::Result<&'static str> {
...
/// Converts ALPACA timeframe string to Nautilus bar specification.
pub fn alpaca_timeframe_as_bar_spec(timeframe: &str) -> anyhow::Result<BarSpecification> {
...
/// Constructs a properly formatted BarType from ALPACA instrument ID and timeframe string.
/// This ensures the BarType uses canonical Nautilus format instead of raw ALPACA strings.
pub fn alpaca_bar_type_from_timeframe(
    instrument_id: InstrumentId,
    timeframe: &str,
) -> anyhow::Result<BarType> {
    let bar_spec = alpaca_timeframe_as_bar_spec(timeframe)?;
    Ok(BarType::new(
        instrument_id,
        bar_spec,
...
/// Converts ALPACA WebSocket channel to bar specification if it's a candle channel.
pub fn alpaca_channel_to_bar_spec(channel: &ALPACAWsChannel) -> Option<BarSpecification> {
    use ALPACAWsChannel::*;
    match channel {
...
/// Parses an ALPACA instrument definition into a Nautilus instrument.
...
/// Common parsed instrument data extracted from ALPACA definitions.
#[derive(Debug)]
...
/// Parses an ALPACA spot instrument definition into a Nautilus currency pair.
...
/// Parses an ALPACA swap instrument definition into a Nautilus crypto perpetual.
pub fn parse_swap_instrument(
    ...
    let is_inverse = match definition.ct_type {
        ALPACAContractType::Linear => false,
        ALPACAContractType::Inverse => true,
        ALPACAContractType::None => {
...
/// Parses an ALPACA futures instrument definition into a Nautilus crypto future.
...
/// Parses an ALPACA option instrument definition into a Nautilus option contract.
pub fn parse_option_instrument(
    ...
    let exchange = Some(Ustr::from("ALPACA"));
...
/// Parses an ALPACA account into a Nautilus account state.
pub fn parse_account_state(
    ...
    for b in &alpaca_account.details {
    ...
    // ALPACA provides account-level margin requirements (not per instrument)
    if !alpaca_account.imr.is_empty() && !alpaca_account.mmr.is_empty() {
        match (
            alpaca_account.imr.parse::<f64>(),
            alpaca_account.mmr.parse::<f64>(),
        ) {
            (Ok(imr_value), Ok(mmr_value)) => {
                if imr_value > 0.0 || mmr_value > 0.0 {
                    let margin_currency = Currency::USD();
                    let margin_instrument_id =
                        InstrumentId::new(Symbol::new("ACCOUNT"), Venue::new("ALPACA"));
    ...
    let ts_event = UnixNanos::from(millis_to_nanos(alpaca_account.u_time as f64));
...
mod tests {
    ...
    use super::*;
    use crate::{
        common::{enums::ALPACAMarginMode, testing::load_test_json},
        http::{
            client::ALPACAResponse,
            models::{
                ALPACAAccount, ALPACABalanceDetail, ALPACACandlestick, ALPACAIndexTicker, ALPACAMarkPrice,
                ALPACAOrderHistory, ALPACAPlaceOrderResponse, ALPACAPosition, ALPACAPositionHistory,
                ALPACAPositionTier, ALPACATrade, ALPACATransactionDetail,
            },
        },
    };
    ...
    fn test_parse_trades() {
        ...
        let parsed: ALPACAResponse<ALPACATrade> = serde_json::from_str(&json_data).unwrap();
        ...
        assert_eq!(trade0.side, ALPACASide::Sell);
        ...
        assert_eq!(trade1.side, ALPACASide::Buy);
    ...
    fn test_parse_candlesticks() {
        ...
        let parsed: ALPACAResponse<ALPACACandlestick> = serde_json::from_str(&json_data).unwrap();
    ...
    fn test_parse_mark_price() {
        ...
        let parsed: ALPACAResponse<ALPACAMarkPrice> = serde_json::from_str(&json_data).unwrap();
    ...
    fn test_parse_index_price() {
        ...
        let parsed: ALPACAResponse<ALPACAIndexTicker> = serde_json::from_str(&json_data).unwrap();
    ...
    fn test_parse_account() {
        ...
        let parsed: ALPACAResponse<ALPACAAccount> = serde_json::from_str(&json_data).unwrap();
    ...
    fn test_parse_order_history() {
        ...
        let parsed: ALPACAResponse<ALPACAOrderHistory> = serde_json::from_str(&json_data).unwrap();
        ...
        assert_eq!(order.state, ALPACAOrderStatus::Filled);
    ...
    fn test_parse_position() {
        ...
        let parsed: ALPACAResponse<ALPACAPosition> = serde_json::from_str(&json_data).unwrap();
        ...
        assert_eq!(pos.pos_side, ALPACAPositionSide::Long);
    ...
    fn test_parse_position_history() {
        ...
        let parsed: ALPACAResponse<ALPACAPositionHistory> = serde_json::from_str(&json_data).unwrap();
        ...
        assert_eq!(hist.inst_type, ALPACAInstrumentType::Swap);
        assert_eq!(hist.mgn_mode, ALPACAMarginMode::Isolated);
        assert_eq!(hist.pos_side, ALPACAPositionSide::Long);
    ...
    fn test_parse_position_tiers() {
        ...
        let parsed: ALPACAResponse<ALPACAPositionTier> = serde_json::from_str(&json_data).unwrap();
    ...
    fn test_parse_account_field_name_compatibility() {
        ...
        let detail_new: ALPACABalanceDetail = serde_json::from_str(&json_new).unwrap();
        ...
        let detail_old: ALPACABalanceDetail = serde_json::from_str(&json_old).unwrap();
    ...
    fn test_parse_place_order_response() {
        ...
        let parsed: ALPACAPlaceOrderResponse = serde_json::from_str(&json_data).unwrap();
        assert_eq!(
            parsed.ord_id,
    ...
    fn test_parse_transaction_details() {
        ...
        let parsed: ALPACATransactionDetail = serde_json::from_str(&json_data).unwrap();
        assert_eq!(parsed.inst_type, ALPACAInstrumentType::Spot);
        ...
        assert_eq!(parsed.side, ALPACASide::Buy);
        assert_eq!(parsed.exec_type, ALPACAExecType::Taker);
    ...
    fn test_parse_spot_instrument() {
        ...
        let response: ALPACAResponse<ALPACAInstrument> = serde_json::from_str(&json_data).unwrap();
        let alpaca_inst: &ALPACAInstrument = response
            .data
            .first()
            .expect("Test data must have an instrument");

        let instrument =
            parse_spot_instrument(alpaca_inst, None, None, None, None, UnixNanos::default()).unwrap();

        assert_eq!(instrument.id(), InstrumentId::from("BTC-USD.ALPACA"));
    ...
    fn test_parse_margin_instrument() {
        ...
        assert_eq!(instrument.id(), InstrumentId::from("BTC-USDT.ALPACA"));
    ...
    fn test_parse_swap_instrument() {
        ...
        let instrument =
            parse_swap_instrument(alpaca_inst, None, None, None, None, UnixNanos::default()).unwrap();

        assert_eq!(instrument.id(), InstrumentId::from("BTC-USD-SWAP.ALPACA"));
    ...
    fn test_parse_futures_instrument() {
        ...
        let instrument =
            parse_futures_instrument(alpaca_inst, None, None, None, None, UnixNanos::default())
                .unwrap();

        assert_eq!(instrument.id(), InstrumentId::from("BTC-USD-241220.ALPACA"));
    ...
    fn test_parse_option_instrument() {
        ...
        let instrument =
            parse_option_instrument(alpaca_inst, None, None, None, None, UnixNanos::default())
                .unwrap();

        assert_eq!(
            instrument.id(),
            InstrumentId::from("BTC-USD-241217-92000-C.ALPACA")
        );
        assert_eq!(
    ...
    fn test_parse_account_state() {
        let json_data = load_test_json("http_get_account_balance.json");
        let response: ALPACAResponse<ALPACAAccount> = serde_json::from_str(&json_data).unwrap();
        let alpaca_account = response
            .data
            .first()
        ...
        let account_state =
            parse_account_state(alpaca_account, account_id, UnixNanos::default()).unwrap();
    ...
    fn test_parse_account_state_with_margins() {
        ...
        let alpaca_account: ALPACAAccount = serde_json::from_str(account_json).unwrap();
        let account_id = AccountId::new("ALPACA-001");
        let account_state =
            parse_account_state(&alpaca_account, account_id, UnixNanos::default()).unwrap();
        ...
        assert_eq!(margin.instrument_id.venue.as_str(), "ALPACA");
    ...
    fn test_parse_account_state_empty_margins() {
        ...
        let alpaca_account: ALPACAAccount = serde_json::from_str(account_json).unwrap();
        let account_id = AccountId::new("ALPACA-SPOT");
        let account_state =
            parse_account_state(&alpaca_account, account_id, UnixNanos::default()).unwrap();
    ...
    fn test_parse_order_status_report() {
        let json_data = load_test_json("http_get_orders_history.json");
        let response: ALPACAResponse<ALPACAOrderHistory> = serde_json::from_str(&json_data).unwrap();
        let alpaca_order = response
            .data
            .first()
    ...
    fn test_parse_position_status_report() {
        let json_data = load_test_json("http_get_positions.json");
        let response: ALPACAResponse<ALPACAPosition> = serde_json::from_str(&json_data).unwrap();
        let alpaca_position = response
            .data
            .first()
    ...
    fn test_parse_trade_tick() {
        let json_data = load_test_json("http_get_trades.json");
        let response: ALPACAResponse<ALPACATrade> = serde_json::from_str(&json_data).unwrap();
        let alpaca_trade = response.data.first().expect("Test data must have a trade");

        let instrument_id = InstrumentId::from("BTC-USDT.ALPACA");
        let trade_tick =
            parse_trade_tick(alpaca_trade, instrument_id, 2, 8, UnixNanos::default()).unwrap();
    ...
    fn test_parse_mark_price_update() {
        let json_data = load_test_json("http_get_mark_price.json");
        let response: ALPACAResponse<crate::http::models::ALPACAMarkPrice> =
            serde_json::from_str(&json_data).unwrap();
        let alpaca_mark_price = response
            .data
            .first()
        ...
        let mark_price_update =
            parse_mark_price_update(alpaca_mark_price, instrument_id, 2, UnixNanos::default())
                .unwrap();
    ...
    fn test_parse_index_price_update() {
        let json_data = load_test_json("http_get_index_price.json");
        let response: ALPACAResponse<crate::http::models::ALPACAIndexTicker> =
            serde_json::from_str(&json_data).unwrap();
        let alpaca_index_ticker = response
            .data
            .first()
        ...
        let index_price_update =
            parse_index_price_update(alpaca_index_ticker, instrument_id, 2, UnixNanos::default())
                .unwrap();
    ...
    fn test_parse_candlestick() {
        let json_data = load_test_json("http_get_candlesticks.json");
        let response: ALPACAResponse<crate::http::models::ALPACACandlestick> =
            serde_json::from_str(&json_data).unwrap();
        let alpaca_candlestick = response
            .data
            .first()
        ...
        let bar = parse_candlestick(alpaca_candlestick, bar_type, 2, 8, UnixNanos::default()).unwrap();
    ...
    fn test_parse_aggressor_side() {
        assert_eq!(
            parse_aggressor_side(&Some(ALPACASide::Buy)),
            AggressorSide::Buyer
        );
        assert_eq!(
            parse_aggressor_side(&Some(ALPACASide::Sell)),
    ...
    fn test_parse_execution_type() {
        assert_eq!(
            parse_execution_type(&Some(ALPACAExecType::Maker)),
            LiquiditySide::Maker
        );
        assert_eq!(
            parse_execution_type(&Some(ALPACAExecType::Taker)),
            LiquiditySide::Taker
        );
    ...
    fn test_fill_report_parsing() {
        ...
        let transaction_detail = crate::http::models::ALPACATransactionDetail {
            inst_type: ALPACAInstrumentType::Spot,
            ...
            side: ALPACASide::Buy,
            exec_type: ALPACAExecType::Taker,
    ...
    fn test_bar_type_identity_preserved_through_parse() {
        ...
        use crate::http::models::ALPACACandlestick;

        // Create a BarType
        let bar_type = BarType::from_str("ETH-USDT-SWAP.ALPACA-1-MINUTE-LAST-EXTERNAL").unwrap();

        // Create sample candlestick data
        let raw_candlestick = ALPACACandlestick(
