...
use nautilus_alpaca::{common::enums::ALPACAInstrumentType, http::client::ALPACAHttpClient};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut client = ALPACAHttpClient::from_env().unwrap();

    // Request instruments
    let inst_type = ALPACAInstrumentType::Swap;
    let instruments = client.request_instruments(inst_type).await?;
    client.add_instruments(instruments);

    let inst_type = ALPACAInstrumentType::Spot;
    let instruments = client.request_instruments(inst_type).await?;
    client.add_instruments(instruments);
    ...
    // Request bars
    let bar_type = BarType::from("BTC-USDT-SWAP.ALPACA-1-MINUTE-LAST-EXTERNAL");
    ...
    //     .instrument_type(ALPACAInstrumentType::Swap)
    //     .trade_mode(ALPACATradeMode::Isolated)
