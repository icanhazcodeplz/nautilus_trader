...
use nautilus_alpaca::{
    common::enums::ALPACAInstrumentType, http::client::ALPACAHttpClient, websocket::ALPACAWebSocketClient,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let http_client = ALPACAHttpClient::from_env().unwrap();
    let instruments = http_client
        .request_instruments(ALPACAInstrumentType::Swap)
        .await?;

    let mut ws_client = ALPACAWebSocketClient::from_env().unwrap();
    ws_client.initialize_instruments_cache(instruments.clone());
    ws_client.connect().await?;

    let instrument_id = InstrumentId::from("BTC-USD-SWAP.ALPACA");

    // let mut client_business = ALPACAWebSocketClient::new(
    //     Some(ALPACA_WS_BUSINESS_URL),
    ...
    ws_client
        .subscribe_instruments(ALPACAInstrumentType::Swap)
        .await?;
